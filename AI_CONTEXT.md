...
- **The Ultimate Escape Route:** If `command` prefixes become too cumbersome or you repeatedly trigger hanging pagers and aliases from the `fish` shell, completely break out of the fish environment by running `exec bash --noprofile --norc`. This instantly replaces the fish process with a clean, raw bash session, completely neutralizing all aliases (e.g., `cat` going back to normal `cat` instead of `bat`) and stopping AI terminal hangs. **Never** use `bash -c` for this, as it executes a subshell and waits (hanging), whereas `exec` properly replaces the running shell.
